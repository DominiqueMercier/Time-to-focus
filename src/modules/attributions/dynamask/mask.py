...
from modules.attributions.dynamask.perturbation import Perturbation
from modules.attributions.dynamask.metrics import get_entropy, get_information
...
class Mask:
    ...
    mask_tensor : torch.tensor
        the tensor containing the mask coefficients
    T : int
    ...
    def __init__(self, perturbation: Perturbation, device, task: str = 'regression',
                 ...
                 deletion_mode: bool = False, eps: float = 1.0e-7, deep: bool = False):
    ...
    def fit(self, X, f, loss_function, target=None,
        ...
        :param target: if the output to approximate is different from f(X), it can be specified optionally
        :param n_epoch: number of steps for the optimization
        :param keep_ratio: fraction of elements in X that should be kept by the mask (called a in the paper)
        ...
        if target is None:
            if not self.deep:
                ...
                self.Y_target = f(X.transpose(0, 1).unsqueeze(0))
                self.Y_target = torch.argmax(self.Y_target.long(), axis=1)
        else:
        ...
        for k in range(n_epoch):
            ...
            if not self.deep:
                ...
                Y_pert = f(X_pert.transpose(0, 1).unsqueeze(0))
    ...
    def get_smooth_mask(self, sigma=1):
        ...
        :return: the smoothed mask (as a torch tensor)
        """
    ...
    def extract_submask(self, mask_tensor, ids_time, ids_feature):
        ...
        :param mask_tensor: the tensor from which data should be extracted
    ...
    def plot_mask(self, ids_time=None, ids_feature=None,
        ...
        :param ids_feature: list of the features that should appear on the plot
        :param smooth: True if the mask should be smoothed before plotting
        :param sigma: width of the smoothing Gaussian kernel
        """
        sns.set()
        ...
        sns.heatmap(data=df, cmap=color_map,
    ...
    def get_error(self):
        ...
        :return: error as a torch scalar
        """
        if self.deletion_mode:
        ...
        if not self.deep:
            ...
            Y_pert = self.f(torch.unsqueeze(X_pert.transpose(0, 1), axis=0))[0]
        if self.task == 'classification':
            Y_pert = torch.log(Softmax(dim=1)(Y_pert))
        if self.deep:
            Y_pert = Y_pert.unsqueeze(0)
