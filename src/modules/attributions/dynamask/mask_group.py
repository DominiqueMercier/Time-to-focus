...
from modules.attributions.dynamask.losses import crossentropy_deep
from modules.attributions.dynamask.mask import Mask
...
class MaskGroup:
    ...
    def __init__(self, device, perturbation: Perturbation, random_seed: int = 987,
                 deletion_mode: bool = False, verbose: bool = False, deep: bool = True):
    ...
    def fit(self, X, f, area_list, loss_function,
            ...
            time_reg_factor: float = 0, verbose: bool = False):
        """
        ...
        if not self.deep:
            ...
            Y_target_group = self.Y_target.clone().detach().unsqueeze(0).repeat(N_area, 1).squeeze(1)
        ...
        for k in range(n_epoch):
            ...
            if not self.deep:
                ...
                Y_pert = torch.stack([f(torch.unsqueeze(x_pert.transpose(0, 1), axis=0))[0] for x_pert in torch.unbind(X_pert, dim=0)], dim=0)
            ...
            optimizer.zero_grad()
            try: #  fixed error during backprop
                loss.backward()
            except:
        ...
        for index, mask_tensor in enumerate(self.masks_tensor.unbind(dim=0)):
            mask = Mask(perturbation=self.perturbation, device=self.device, verbose=False,
                        deletion_mode=self.deletion_mode, deep=self.deep)
    ...
    def get_best_mask(self, verbose=0):
        """
    ...
    # attribute version to have similar shape as captum
    def attribute(self, x, y, model, area_list, n_epoch=1000):
        attr = []
        for data, label in zip(x, y):
            self.fit(f=model, X=data.T, area_list=area_list, loss_function=crossentropy_deep, n_epoch=n_epoch)
            mask = self.get_best_mask()
            sal = mask.mask_tensor.clone().detach().cpu().T
